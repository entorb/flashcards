...
- `BASE_PATH = 'fc-div'`
- `DEFAULT_RANGE = [2, 3, 4, 5, 6, 7, 8, 9]` (all selectable and selected by default)
...
- `src/services/storage.ts` — `initializeCards()`, `getVirtualCards()` (accepts optional `storedCards` param — pass the page's loaded cards so computed re-renders after reset), `parseCardQuestion()`
