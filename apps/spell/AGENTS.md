# Eisi's Lernwörter - Spelling Trainer App
...
**BASE_PATH:** `spell` (router base, storage keys, DB collection)
...
```typescript
'spell-decks' // CardDeck[]
'spell-history' // GameHistory[]
'spell-stats' // GameStats
'spell-game-config' // GameSettings (sessionStorage)
'spell-game-result' // GameHistory (sessionStorage)
'spell-daily-stats' // DailyStats
```
...
- All text in `packages/shared/src/text-de.ts` under `TEXT_DE.spell.*`
- Storage keys prefix: `spell-`
