# Flashcards Apps

This repository hosts the code of six educational apps, implementing flashcard-based learning for school kids. Hosted at <https://entorb.net/flashcards/>:
<!-- rumdl-disable no-inline-html -->

<table>
<thead>
<tr>
<th align="center">
<a href="https://entorb.net/fc-1x1/">1×1 Trainer</a>
</th>
<th align="center">
<a href="https://entorb.net/fc-div/">Division</a>
</th>
<th align="center">
<a href="https://entorb.net/fc-voc/">Vokabeltrainer</a>
</th>
<th align="center">
<a href="https://entorb.net/fc-lwk/">Lernwörter</a>
</th>
<th align="center">
<a href="https://entorb.net/fc-pum/">Plus & Minus</a>
</th>
<th align="center">
<a href="https://entorb.net/fc-eta/">Hausi-Timer</a>
</th>
</tr>
</thead>
<tbody>
<tr>
<td align="center">
<a href="https://entorb.net/fc-1x1/">
<img src="apps/1x1/assets/icon.svg" alt="1×1 Murmeltier">
</a>
</td>
<td align="center">
<a href="https://entorb.net/fc-div/">
<img src="apps/div/assets/icon.svg" alt="Division Huhn">
</a>
</td>
<td align="center">
<a href="https://entorb.net/fc-voc/">
<img src="apps/voc/assets/icon.svg" alt="Vokabeltrainer Fuchs">
</a>
</td>
<td align="center">
<a href="https://entorb.net/fc-lwk/">
<img src="apps/lwk/assets/icon.svg" alt="Lernwörter Eisbär">
</a>
</td>
<td align="center">
<a href="https://entorb.net/fc-pum/">
<img src="apps/pum/assets/icon.svg" alt="Plus & Minus Waschbär">
</a>
</td>
<td align="center">
<a href="https://entorb.net/fc-eta/">
<img src="apps/eta/assets/icon.svg" alt="Hausi-Timer">
</a>
</td>
</tbody>
</table>

- Lernmethode: Karteikarten-System mit 5 Leveln (Level 1 = unbekannt → Level 5 = sicher beherrscht)
- Karten: Rechenaufgaben (1×1) • Division • Plus & Minus • Vokabeln mit Übersetzung • Lernwörter für Rechtschreibung
- Kisten: Karten werden in Kisten organisiert (voc & lwk), z.B. pro Kapitel oder Sprache
- Spielmodi: Normal (max. 10 Karten) • Endlos Level 1 • 3 Runden • Endlos bis Level 5
- Fokus: Gewichtete Kartenauswahl (Schwache oder Langsame)
- Spielarten: Multiple Choice / Blind / Tippen (voc) • Abschreiben / Verdeckt (lwk)
- Gamification: Punkte sammeln • Maskottchen freut sich • Tagesstatistik mit Bonuspunkten • Spielverlauf-Historie
- Technologie: Web • offline nutzbar • PWA App
  (kann auf Android oder Apple als App installiert werden)
- Privatsphäre: Open Source • Keine Kosten • Keine Werbung • Kein Login • Keine Datenspeicherung auf dem Server

## Tech Stack

- pnpm
- Vue.js
- Quasar Framework, no custom styles
- TypeScript with strict rules
- Biome (formatting)
- ESLint (linting)
- Vitest unit tests
- Cypress E2E test
- [SonarQube](https://sonarcloud.io/summary/overall?id=entorb_flashcards&branch=main) for
  code quality analysis

## Scripts

```sh
# Development
pnpm install             # Install all dependencies
pnpm dev                 # Run both apps in parallel
pnpm dev:1x1             # Run only 1x1 app
pnpm dev:div             # Run only div app
pnpm dev:voc             # Run only voc app
pnpm dev:lwk             # Run only lwk app
pnpm dev:pum             # Run only pum app
pnpm dev:eta             # Run only eta app

# Build
pnpm build               # Build all apps
pnpm build:1x1           # Build only 1x1 app
pnpm build:div           # Build only div app
pnpm build:voc           # Build only voc app
pnpm build:lwk           # Build only lwk app
pnpm build:pum           # Build only pum app
pnpm build:eta           # Build only eta app

# Preview
pnpm preview             # Preview all apps in parallel
pnpm preview:1x1         # Preview only 1x1 app
pnpm preview:div         # Preview only div app
pnpm preview:voc         # Preview only voc app
pnpm preview:lwk         # Preview only lwk app
pnpm preview:pum         # Preview only pum app
pnpm preview:eta         # Preview only eta app

# Quality Checks
pnpm run types               # Type check all apps and packages
pnpm run test                # Run tests in all apps
pnpm run test:cov            # Run tests with coverage
./scripts/run_checks.sh      # Full check suite (format, lint, types, tests, spelling, dead code, …)

# run only for certain app, here 1x1
pnpm --filter 1x1 run test

# Testing Vitest Unit Tests
pnpm run test

# Testing Cypress E2E Tests
pnpm run cy:open:1x1
pnpm run cy:open:div
pnpm run cy:open:voc
pnpm run cy:open:lwk
pnpm run cy:open:pum
pnpm run cy:open:eta
# headless
pnpm run cy:run
pnpm run cy:run:1x1
pnpm run cy:run:div
pnpm run cy:run:voc
pnpm run cy:run:lwk
pnpm run cy:run:pum
pnpm run cy:run:eta

# Testing Playwright E2E Tests
pnpm run pw:open:1x1
pnpm run pw:open:div
pnpm run pw:open:voc
pnpm run pw:open:lwk
pnpm run pw:open:pum
pnpm run pw:open:eta
# headless
pnpm run pw:run
pnpm run pw:run:1x1
pnpm run pw:run:div
pnpm run pw:run:voc
pnpm run pw:run:lwk
pnpm run pw:run:pum
pnpm run pw:run:eta

# Maintenance
pnpm clean               # Remove all node_modules and dist folders
pnpm clean:cache         # Remove cache files
```
