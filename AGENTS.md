...
- When the same UI pattern (e.g., toggle buttons, filter controls) appears on multiple pages within one app, extract it into a reusable component in that app's `src/components/` — don't copy-paste the template + logic across pages
